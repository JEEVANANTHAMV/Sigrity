...
TOOL_STATUS_NOTES: dict[str, str] = {
    ...
    "alternative either. A NEW, DISTINCT intermittent failure mode surfaced via the "
    "vLLM-driven manufacturing eval (scripts/eval_manufacturing.py, node-11 run): "
    "`allegro_run_session` returned `state=failed, returncode=-536870904` after almost "
    "exactly ~137-138s, with run.log showing only the startup banner -- the same "
    "specific negative code and ~137s duration this session saw multiple times during "
    "its own diagnostic testing before the stale-lock root cause (above) was found and "
    "fixed. This particular occurrence happened with NO overlapping Allegro launches "
    "(strictly sequential harness, board freshly copied) and no pre-existing lock file, "
    "so it is a genuinely separate, still-not-fully-isolated cause -- possibly a "
    "different modal dialog, a transient license-server hiccup, or something else "
    "entirely; the consistent ~137s timing across every occurrence suggests a real "
    "internal watchdog/timeout rather than a random crash. The model handled it well: "
    "it correctly called the new `check_design_lock` diagnostic tool afterward (found a "
    "lock the failed run itself had orphaned), then successfully continued the rest of "
    "the pipeline. Treat an Allegro batch launch that's still `running` well past its "
    "usual 15-20s load time, or that fails with this exact code, as a known, real, "
    "empirically-recurring (roughly 1-in-4 launches across this session) reliability "
    "issue -- not yet fully root-caused, retry rather than treat as a hard blocker.",
