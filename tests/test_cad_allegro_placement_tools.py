...
@pytest.mark.asyncio
async def test_run_allegro_zrouter_positional_args(fake_exe):
    result = await run_allegro_zrouter("board.brd", "control.txt", output_file="out.brd")
    assert result["command"][1:] == ["control.txt", "board.brd", "out.brd"]
